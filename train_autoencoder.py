...
def get_dataset_location_DL(model_folder, DLC_folder):

    dlc_parent_folder, _ = os.path.split(model_folder)
    # dlc_folder_testname = os.path.join(dlc_parent_folder, DLC_folder)
    DLC_path = os.path.join(dlc_parent_folder, DLC_folder)

    config_fname = os.path.join(DLC_path, 'config.yaml')
    ...
    fname_pat = os.path.join(
        DLC_path, 'training-datasets', 'iteration-'+str(iternum),
        '*', 'CollectedData_*.h5')
    fname = glob(fname_pat)[0]
...
def train_autoencoder(config, DLC_folder):
    '''
    ...
    data_fname = get_dataset_location_DL(model_folder, DLC_folder)
    ...
    dlc_folder_parts = DLC_folder.split('-')
    dlc_proj_name = dlc_folder_parts[0]
    out_fname = '_'.join((dlc_proj_name, 'autoencoder.pickle'))
    out_fname = os.path.join(config['model_folder'], out_fname)
