...
class CameraGroup:
    ...
    def bundle_adjust_iter_fixed_intrinsics(self, p2ds, extra=None,
                           n_iters=10, start_mu=15, end_mu=1,
                           max_nfev=200, ftol=1e-4,
                           n_samp_iter=100, n_samp_full=1000,
                           error_threshold=0.3,
        ...
        for i in range(n_iters):
            ...
            self.bundle_adjust_fixed_intrinsics(p2ds_samp, extra_samp,
                               loss='linear', ftol=ftol,
                               max_nfev=max_nfev,
                               verbose=verbose)
    ...
    def bundle_adjust_iter_fixed_dist(self, p2ds, extra=None,
        ...
        p2ds, extra = resample_points(p2ds_full, extra_full,
                                      n_samp=n_samp_full)
        error = self.average_error(p2ds, undistort=False, median=True)
        ...
        for i in range(n_iters):
            ...
            self.bundle_adjust_fixed_dist(p2ds_samp, extra_samp,
                               loss='linear', ftol=ftol,
                               max_nfev=max_nfev,
                               verbose=verbose)
    ...
    def bundle_adjust(self, p2ds, extra=None,
        ...
        x0, n_cam_params = self._initialize_params_bundle(p2ds, extra, undistort=False)
    ...
    def bundle_adjust_fixed_dist(self, p2ds, extra=None,
                      loss='linear',
                      threshold=50,
                      ftol=1e-4,
                      max_nfev=1000,
        ...
        x0, n_cam_params = self._initialize_params_bundle_fixed_dist(p2ds, extra, undistort=False)
    ...
    def bundle_adjust_fixed_intrinsics(self, p2ds, extra=None,
                      loss='linear',
                      threshold=50,
                      ftol=1e-4,
                      max_nfev=1000,
        ...
        x0, n_cam_params = self._initialize_params_bundle_fixed_intrinsics(p2ds, extra, undistort=False)
    ...
    def bundle_adjust_fixed_intrinsics_and_cam0(self, p2ds, extra=None,
                      loss='linear',
                      threshold=50,
                      ftol=1e-4,
                      max_nfev=1000,
        ...
        x0, n_cam_params = self._initialize_params_bundle_fixed_intrinsics_and_cam0(p2ds, extra, undistort=False)
    ...
    def _error_fun_bundle(self, params, p2ds, n_cam_params, extra):
        ...
        for i in range(n_cams):
            ...
            # params[7] = dist[0] and params[8]=dist[1]
    ...
    @jit(parallel=True, forceobj=True)
    def _error_fun_bundle_fixed_dist(self, params, p2ds, n_cam_params, extra):
        """Error function for bundle adjustment"""
        good = ~np.isnan(p2ds)
        n_cams = len(self.cameras)
        ...
        for i in range(n_cams):
            ...
            # workaround to force this distortion coefficients to zero since they're no longer included in the
    ...
    @jit(parallel=True, forceobj=True)
    def _error_fun_bundle_fixed_intrinsics_and_cam0(self, params, p2ds, n_cam_params, extra):
        """Error function for bundle adjustment"""
        good = ~np.isnan(p2ds)
        n_cams = len(self.cameras)
        ...
        sub = n_cam_params * (n_cams - 1)   # subtract one because we're fixing camera 0
        n3d = p2ds.shape[1] * 3
        p3ds_test = params[sub:sub+n3d].reshape(-1, 3)
        errors = self.reprojection_error(p3ds_test, p2ds)
        errors_reproj = errors[good]
    ...
    def _jac_sparsity_bundle(self, p2ds, n_cam_params, extra):
        ...
        for i in range(3):
            A_sparse[ix, (n_cams-1) * n_cam_params + point_indices_good * 3 + i] = 1
    ...
    def _jac_sparsity_bundle_fixed_intrinsics_and_cam0(self, p2ds, n_cam_params, extra):
        """Given an CxNx2 array of 2D points,
        where N is the number of points and C is the number of cameras,
        compute the sparsity structure of the jacobian for bundle adjustment"""
    ...
    def _initialize_params_bundle(self, p2ds, extra, undistort=False):
        """Given an CxNx2 array of 2D points,
        where N is the number of points and C is the number of cameras,
    ...
    def _initialize_params_bundle_fixed_dist(self, p2ds, extra, undistort=False):
        """Given an CxNx2 array of 2D points,
        where N is the number of points and C is the number of cameras,
        initializes the parameters for bundle adjustment"""

        # only take the first 6 params - ignore the distortion coefficients
        ...
        # consider getting rid of the focal length as a parameter as well, since it should be the same for each virtual camera
    ...
    def _initialize_params_bundle_fixed_intrinsics(self, p2ds, extra, undistort=False):
        """Given an CxNx2 array of 2D points,
        where N is the number of points and C is the number of cameras,
        initializes the parameters for bundle adjustment"""
    ...
    def _initialize_params_bundle_fixed_intrinsics_and_cam0(self, p2ds, extra, undistort=False):
        """Given an CxNx2 array of 2D points,
        where N is the number of points and C is the number of cameras,
        initializes the parameters for bundle adjustment"""
        ...
        # get rid of the focal length as a parameter as well, since it should be the same for each virtual camera
        # we're going to assume camera 0 is fixed at zero rotation and zero translation; its lens is the origin
        cam_params = np.hstack([cam.get_params()[:6] for cam in self.cameras[1:]])
        n_cam_params = len(cam_params) // (len(self.cameras) - 1)
    ...
    def average_error(self, p2ds, median=False, undistort=True):
        p3ds = self.triangulate(p2ds, undistort=undistort)
    ...
    def calibrate_rows_fixed_dist(self, all_rows, board,
                       init_intrinsics=True, init_extrinsics=True, verbose=True,
                       **kwargs):
        assert len(all_rows) == len(self.cameras), \
            "Number of camera detections does not match number of cameras"
        ...
        error = self.bundle_adjust_iter_fixed_dist(imgp, extra, verbose=verbose, **kwargs)
